checkpoint download_MEGA_physical_genetic_coordinates:
	...
	# benchmark:
	# 	repeat("benchmarks/download_MEGA_physical_genetic_coordinates.array", 3)
	shell:
		"""
...
# Step 3 -
# Create cleanest possible regions file from Multi-EthnicGlobal_D1.csv_Physical-and-Genetic-Coordinates.txt
rule cleanRegionsFile:
	...
	# benchmark:
	# 	repeat("benchmarks/cleanRegionsFile.array", 3)
	shell:
		"awk '{{print $2,$3}}' {input.coord} | "
		"sed 's/ /-/g' - | " # might be '\t' for GRCh37
		"sort -n - | "
		"sed '/0-0/d' - | sed '/XY/d' - | sed '/MT/d' - | sed '/Chr-MapInfo/d' | " # Used to do MANUAL CLEANING OF FILE (remove the 0-0, XY [not X, or Y, but XY], and MT) # thoroughly check this code
		"sed 's/-/\t/g' - > {output.regionsFile}"

rule extract_clean_regions: # put this into python script
	input:
		regionsFile="data/snp_array/vcf/D2_cleanRegionsFile.txt",
	...
	# benchmark:
	# 	repeat("benchmarks/extract_clean_regions.array", 3)
	script:
...
checkpoint download_lociNames_conversion_file:
	...
	params:
		...
		supportDir="illumina/support_files/",
	# benchmark:
	# 	repeat("benchmarks/download_lociNames_conversion_file.array", 3)
	shell:
		"""
...
rule replace_lociNames_to_rsIDs:
	...
	# benchmark:
	# 	repeat("benchmarks/replace_lociNames_to_rsIDs.array", 3)
	script:
...
rule original_lociNames:
	...
	# benchmark:
	# 	repeat("benchmarks/original_lociNames.array", 3)
	shell:
...
rule insert_rsIDS:
	...
	# benchmark:
	# 	repeat("benchmarks/insert_rsIDS.array", 3)
	shell:
		"""
...
rule filter_for_dbSNP_vars:
	...
	# benchmark:
	# 	repeat("benchmarks/filter_for_dbSNP_vars.array", 3),
	resources:
		mem_mb=59900,
	script:
